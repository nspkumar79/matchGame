...
const initialFilterList = imagesList.filter(
...
class App extends Component {
  ...
  onClicked = id => {
    const filterImagesList = imagesList.filter(
      eachItem => eachItem.category === id,
    )
    this.setState({filterList: filterImagesList, tabId: id})
  }
  ...
  render() {
    const {isThumbClicked, filterList, tabId} = this.state
    return (
      <>
        <div className="navbar-container">
          ...
          <div className="nav-text-container">
            <p className="nav-text">
              Score:<span className="highlight-text">0</span>
            </p>
            <div className="timer-container">
              <img
                ...
                alt="timer"
              />
              <p className="highlight-text">60 sec</p>
            </div>
          </div>
        </div>
            ...
            <ul className="tab-list-container">
              {tabsList.map(eachItem => (
                <TabItem
                  ...
                  isTabActive={eachItem.tabId === tabId}
                />
              ))}
            </ul>
            <ul className="thumnail-list-container">
              {filterList.map(eachItem => (
                <ThumbnailItem
                  eachItem={eachItem}
                  key={eachItem.id}
                  onThumbClicked={this.onThumbClicked}
                />
              ))}
            </ul>
          </div>
